...
module.exports = {
     ...
     * @param {Object} param0
     */
    name: 'config-counting',
    description: 'Configure your your Counting Channel',
    options:
        [{
            name: 'add',
    ...
    callback: async(client, interaction) => {
        ...
        if(interaction.options.getSubcommand() === 'add'){
            ...
            if(guildConfiguration.moderationChannelIds.includes(channel.id)){
                await interaction.reply(`${channel} ${language(guild, 'CONFIG_COUNT_ALREADY')}`);
                return;
            }
            ...
            await interaction.reply(`${channel} ${language(guild, 'CONFIG_COUNT_ADDED')}`);
            return;
        };
        ...
        if(interaction.options.getSubcommand() === 'remove'){
            ...
            if(!guildConfiguration.moderationChannelIds.includes(channel.id)){
                await interaction.reply(`${channel} ${language(guild, 'CONFIG_COUNT_NOT')}`);
                return;
            }
            ...
            await interaction.reply(`${channel} ${language(guild, 'CONFIG_COUNT_REM')}`);
            return;
        }
    }
